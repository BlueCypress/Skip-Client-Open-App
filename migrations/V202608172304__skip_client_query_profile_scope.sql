...
DECLARE @QueryScopeID UNIQUEIDENTIFIER =
    (SELECT [ID] FROM [${mjSchema}].[APIScope] WHERE [FullPath] = 'query');
...
IF @QueryScopeID IS NULL
BEGIN
    ROLLBACK TRANSACTION;
    THROW 50000, 'Skip Client: the MJ core API scope ''query'' was not found. Deploy the MJ API scope catalog before installing this app.', 1;
END

IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[APIScope] WHERE [ID] = '5FB4019D-7DBA-45A3-BDD9-B7995F44073C')
BEGIN
...
DECLARE @MJAPIAppID UNIQUEIDENTIFIER =
    (SELECT [ID] FROM [${mjSchema}].[APIApplication] WHERE [Name] = 'MJAPI');

IF @MJAPIAppID IS NULL
BEGIN
    ...
    PRINT 'Skip Client: API application ''MJAPI'' not found — skipping the query:profile ceiling grant. Query profiling will be denied until an application-scope grant is added.';
END
ELSE IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[APIApplicationScope] WHERE [ID] = 'F7162165-9589-4511-9F4B-4B46EE58CC36')
BEGIN
